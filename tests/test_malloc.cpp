...
#define MALLOC_TEST_COUNT 200000
...
void test_custom_allocator()
{
    ...
    for (size_t i = 0; i < MALLOC_TEST_COUNT; i++)
    {
        ...
        MemoryManager::ptr<SmallTest> t2 = MemoryManager::Manager::getInstance().allocate<SmallTest>();
    }
...
void test_custom_allocator_array()
{
    ...
    for (size_t i = 0; i < MALLOC_TEST_COUNT; i++)
    {
        MemoryManager::ptr<SmallTest> t1 = MemoryManager::Manager::getInstance().allocate<SmallTest>(200);
        MemoryManager::ptr<SmallTest> t2 = MemoryManager::Manager::getInstance().allocate<SmallTest>(200);
    }
    auto stop = high_resolution_clock::now();
    auto duration = duration_cast<microseconds>(stop - start);
